...
- Preserve the current blocked status. Compaction never unblocks a goal or grants authority.

## Reload authority in order

1. Read the current live user message completely.
2. Resolve the authoritative living-goal path from explicit attachments or the last user designation.
...
- current blockers, pending user decisions, active workers, and expected next event;
...
- If the full chain cannot be reconstructed uniquely, keep or mark the goal blocked and request the missing decision.
