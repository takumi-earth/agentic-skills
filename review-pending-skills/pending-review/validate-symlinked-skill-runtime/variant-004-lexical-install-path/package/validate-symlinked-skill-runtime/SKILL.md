...
Apply the `variant-004-lexical-install-path` design without silently merging it with sibling approaches.
...
- Prove registered symlink path.
- Prove PATH lookup.
- Prove direct script invocation.
- Prove renamed wrapper.
- Prove canonical registration negative.
...
- Read `references/lexical-path-limitations.md` when its named contract is load-bearing.
