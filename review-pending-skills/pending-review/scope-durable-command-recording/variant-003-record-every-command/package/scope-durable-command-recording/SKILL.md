...
Apply the `variant-003-record-every-command` design without silently merging it with sibling approaches.
...
- Prove measure wrapper overhead.
- Prove reproduce unsupported-flag failure.
- Prove confirm pending inactive status.
...
- Read `references/record-every-command-tradeoffs.md` when its named contract is load-bearing.
