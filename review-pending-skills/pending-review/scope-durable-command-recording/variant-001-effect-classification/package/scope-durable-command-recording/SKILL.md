...
Apply the `variant-001-effect-classification` design without silently merging it with sibling approaches.
...
- Prove passive SKILL.md read.
- Prove source search with computed selection.
- Prove multi-file mutation.
- Prove Git-state decision.
- Prove ambiguous pipeline.
...
- Read `references/effect-classification.md` when its named contract is load-bearing.
- Run `scripts/classify_command_recording.py` for the deterministic operation it owns; use its `--self-test` before relying on it.
