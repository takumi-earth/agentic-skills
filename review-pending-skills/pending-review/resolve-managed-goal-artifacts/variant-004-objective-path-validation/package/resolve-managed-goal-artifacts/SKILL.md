...
Apply the `variant-004-objective-path-validation` design without silently merging it with sibling approaches.
...
- Prove exact current wrapper string.
- Prove spaces and punctuation in file names.
- Prove two path tokens.
- Prove root mismatch.
- Prove path traversal and symlink escape.
...
- Read `references/objective-path-grammar.md` when its named contract is load-bearing.
