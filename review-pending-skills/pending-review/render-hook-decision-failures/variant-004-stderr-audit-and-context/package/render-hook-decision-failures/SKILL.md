...
Apply the `variant-004-stderr-audit-and-context` design without silently merging it with sibling approaches.
...
- Prove stdout parses independently.
- Prove stderr event contains matching diagnostic code.
- Prove exit zero remains non-blocking.
- Prove additionalContext remains actionable when stderr is discarded.
...
- Read `references/hook-channel-contract.md` when its named contract is load-bearing.
