...
Apply the `variant-003-stage-code-value-map` design without silently merging it with sibling approaches.
...
- Prove stable field ordering.
- Prove home-path normalization.
- Prove null and collection rendering.
- Prove no generic fallback when known values exist.
...
- Read `references/diagnostic-codes.md` when its named contract is load-bearing.
